...
export default function SummonerPage() {
  ...
  const { region, riotId } = useParams<{ region: Platform; riotId: string }>();
  ...
  // 1) Resolver PUUID (una sola vez)
  useEffect(() => {
    (async () => {
      if (!puuid && region && gameName && tagLine) {
        try {
          const { data } = await axiosInstance.get('/api/stats/resolve', { params: { region, gameName, tagLine } });
          setPuuid(data.puuid);
        } catch (err) {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        setIsLoading(true);
        const url = `/api/stats/summary/${region}/${puuid}`;
  ...
  useEffect(() => {
    (async () => {
      if (!puuid || !region) return;
      try {
        const url = `/api/stats/recent/${region}/${puuid}`;
  ...
  // 4) Historial de partidas (IDs + detalle)
  useEffect(() => {
    (async () => {
      if (!puuid || !region) return;
      try {
  ...
  // 5) Verificar si está en partida (tolerante a 204/404)
  useEffect(() => {
    (async () => {
      if (!puuid || !region) return;
      try {
        const resp = await axiosInstance.get(`/api/stats/spectator/${region}/${puuid}`, {
          validateStatus: () => true, // evita excepción en 204/404
        });
  ...
  // 6) Estadísticas por campeón
  useEffect(() => {
    (async () => {
      if (!puuid || !region) return;
      try {
        const { data } = await axiosInstance.get(`/api/stats/champion-stats/${region}/${puuid}`, {
          params: { count: 20 },
        });
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
            ...
            <div className="flex-grow">
              ...
              <div className="flex flex-wrap gap-3 mb-3">
                ...
                <Badge variant="outline" className="border-blue-500 text-blue-300">{region?.toUpperCase()}</Badge>
                {liveGame && (
              ...
              <p className="text-blue-200 text-sm">
                PUUID: {puuid ? puuid.slice(0, 12) + '…' : 'resolviendo…'}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <TabsList className="grid grid-cols-2 md:grid-cols-5 gap-2 p-1 bg-gray-800/50 rounded-lg">
            ...
            <TabsTrigger value="partidas" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white">
              <Swords className="w-4 h-4 mr-2" /> Partidas
            </TabsTrigger>
            <TabsTrigger value="campeones" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white">
            ...
            <TabsTrigger value="roasts" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white">
              <MessageCircle className="w-4 h-4 mr-2" /> Roasts
            </TabsTrigger>
          </TabsList>

          {/* === RESUMEN: Top Maestría + Rendimiento Reciente === */}
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-semibold text-white">
                                {champ?.name ?? String(m.championId)}
                              </p>
                  ...
                  <div className="divide-y divide-gray-700">
                    {recent.slice(0, 5).map((c, idx) => (
                      <div key={c.championId} className="flex items-center justify-between p-4 hover:bg-gray-700/30 transition-colors">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-gradient-to-b from-blue-700 to-blue-500 rounded-lg flex items-center justify-center text-white font-bold">
                            #{idx+1}
                          </div>
                          <div>
                  ...
                  <div className="space-y-4">
                    <div>
                      <div className="flex justify-between text-sm mb-1">
                        <span>Win Rate</span>
                        <span>{overallStats.winRate.toFixed(1)}%</span>
                      </div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div className="text-center p-3 bg-gray-700/50 rounded-lg">
                        <div className="text-xl font-bold text-blue-300">{overallStats.avgKills}</div>
                      ...
                      <div className="text-center p-3 bg-gray-700/50 rounded-lg">
                        ...
                        <div className="text-xs text-gray-400">Muertes</div>
                      </div>
                      <div className="text-center p-3 bg-gray-700/50 rounded-lg">
                        <div className="text-xl font-bold text-purple-300">{overallStats.avgAssists}</div>
              ...
              <CardContent className="p-0">
                <div className="divide-y divide-gray-700">
                  {matchHistory.map((match) => {
                    ...
                    return (
                      <div key={match.matchId} className={`p-4 hover:bg-gray-700/30 transition-colors ${match.win ? 'border-l-4 border-l-green-500' : 'border-l-4 border-l-red-500'}`}>
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex-grow">
                            <div className="flex justify-between items-center">
                              ...
                              <span className={`px-2 py-1 rounded text-xs font-medium ${match.win ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300'}`}>
                                {match.win ? 'Victoria' : 'Derrota'}
                              </span>
                            </div>
                            <p className="text-sm text-gray-400">{new Date(match.gameStartTimestamp).toLocaleDateString()}</p>
                            <div className="flex gap-4 mt-1 text-xs">
                              <span className="text-blue-300">{match.kills}/{match.deaths}/{match.assists}</span>
                  ...
                  {!matchHistory.length && <p className="p-4 text-sm text-gray-400">No hay partidas recientes</p>}
                </div>
              </CardContent>
            </Card>
          </TabsContent>

          {/* === CAMPEONES: Estadísticas por campeón === */}
          <TabsContent value="campeones" className="space-y-4">
            <Card className="bg-gray-800/30 border-gray-700">
              <CardHeader className="bg-gradient-to-r from-blue-900/50 to-purple-900/50 border-b border-gray-700">
                ...
                <CardDescription className="text-gray-400">Rendimiento con cada campeño</CardDescription>
              </CardHeader>
              <CardContent>
                {championStats ? (
...
/>
          ...
          {/* === EN VIVO: Información de partida en curso === */}
          ...
          {/* === ROASTS: Sistema de comentarios === */}
          <TabsContent value="roasts" className="space-y-4">
            <Card className="bg-gray-800/30 border-gray-700">
              <CardHeader className="bg-gradient-to-r from-blue-900/50 to-purple-900/50 border-b border-gray-700">
                <CardTitle className="flex items-center gap-2 text-blue-200">
                  <MessageCircle className="h-5 w-5" />
                  Roasts de la Comunidad
                </CardTitle>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                    ...
                    <textarea 
                      className="w-full bg-gray-700 text-white p-3 rounded-lg border border-gray-600 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" 
                      rows={3}
